...
int so_fflush(SO_FILE *stream)
{
	...
	/*nu e nimic de scris pe disk*/
	if (stream->cursorPos == 0)
		return 0;
	/*trebuie sa fie o eroare? obs- test*/
	stream->flag = 1;
	return EOF;
...
{
	/*intre un read si write mereu un flush, +fseek*/
	so_fflush(stream);
...
int so_fgetc(SO_FILE *stream)
{
	...
	if (stream->cursorPos == stream->cursorSizeBuff
		|| stream->cursorPos == 0) {
		total_bytes = read(stream->fd, stream->buffer, SIZEBUFFER);
		if (total_bytes > 0) {
			if (stream->cursorPos == stream->cursorSizeBuff)
				stream->read_bytes += total_bytes;

			stream->cursorSizeBuff = total_bytes;
			stream->cursorPos = 0;
	...
	character = (int)stream->buffer[stream->cursorPos];
